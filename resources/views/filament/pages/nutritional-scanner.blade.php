<x-filament-panels::page>
    ...
    <style>
        /* (Mantenha seus estilos de layout/cores aqui) */
    ...
    <div class="app-container">
        ...
        {{-- 
           INPUT OCULTO GLOBAL
           wire:model.live é essencial aqui. Ele envia a foto assim que selecionada.
        --}}
        <input type="file" 
        ...
        {{-- ========================================================= --}}
        {{-- ESTADO 1: SCANNER --}}
        {{-- Mostra apenas se step for 'scan' --}}
        {{-- ========================================================= --}}
        <div id="scanner-view" class="{{ $step === 'scan' ? '' : 'hidden' }}" wire:ignore>
            <div id="reader"></div>
            <div class="scan-frame"><div class="scan-line"></div></div>
            <div style="position: absolute; bottom: 15%; width: 100%; text-align: center; color: white;">
                Aponte para o código de barras
            </div>
        </div>

        {{-- ========================================================= --}}
        {{-- ESTADO 2 e 3: VISUALIZAÇÃO DE PRODUTO / FOTO --}}
        {{-- Renderizado pelo PHP (Server Side) --}}
        {{-- ========================================================= --}}
        ...
        <div class="full-screen-layer">
            ...
            <div class="viewport-area">
                ...
                {{-- CENÁRIO A: CONFIRMAÇÃO (Botão de Câmera) --}}
                @if($step === 'confirm')
                    <div style="text-align: center;">
                        
                        {{-- Botão Físico --}}
                        <div wire:loading.remove wire:target="photo">
                            <p style="color: #666; margin-bottom: 20px;">Confirme o produto para capturar.</p>
                        ...
                        {{-- Loading State (Enquanto a foto sobe) --}}
                        <div wire:loading wire:target="photo" style="color: var(--primary);">
                            <x-heroicon-o-arrow-path class="w-10 h-10 animate-spin mx-auto"/>
                            <p class="mt-4 font-bold">Processando Imagem...</p>
                        </div>
                    </div>
                @endif

                {{-- CENÁRIO B: PREVIEW (Foto Carregada) --}}
                @if($step === 'preview')
                    <div style="width: 100%; height: 100%; display: flex; flex-direction: column; align-items: center;">
                        
                        {{-- Mostra a foto temporária (nova) ou a do banco (antiga) --}}
                        @if ($photo)
                        ...
                        <button type="button" onclick="document.getElementById('native-camera-input').click()" 
                                class="text-gray-400 text-xs mt-4 underline p-2">
                            Tirar outra foto
                        </button>
                    </div>
                @endif
            ...
            <div class="controls-bar">
                ...
                {{-- Botão Salvar (Só aparece no Preview) --}}
                @if($step === 'preview')
    ...
    {{-- Script Simples apenas para o Scanner --}}
    <script>
        document.addEventListener('livewire:initialized', () => {
            let html5QrCode = null;
            
            async function startScanner() {
                // Se não estiver no modo scan, para tudo.
            ...
            // Reinicia quando o Livewire mandar o evento de reset
