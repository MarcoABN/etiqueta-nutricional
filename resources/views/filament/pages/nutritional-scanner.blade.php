<x-filament-panels::page>
    ...
    <style>
        ...
        /* Foto */
        #photo-view { position: absolute; inset: 0; z-index: 40; background: #111; display: flex; flex-direction: column; color: white; }
        ...
        .btn-save { background: #22c55e; flex: 2; height: 55px; border-radius: 12px; color: #000; font-weight: bold; transition: all 0.3s; }
        .btn-save:disabled { background: #1a5e32 !important; color: #555 !important; opacity: 0.5; }
        ...
        @if($foundProduct)
            <div id="photo-view">
                ...
                <div class="flex-1 flex flex-col items-center justify-center p-6 text-center">
                    ...
                    <h3 id="status-title" class="text-lg font-bold">Tirar Foto</h3>
                    <p id="status-text" class="text-gray-400 text-sm mb-8">Aponte para a tabela nutricional</p>

                    <button type="button" onclick="triggerCamera()" class="btn-capture">
        ...
        document.addEventListener('livewire:initialized', () => {
            ...
            window.triggerCamera = function() {
                ...
                if (fileInput) {
                    ...
                    // Fallback: Se após 5 segundos não houver resposta do FilePond, 
                    // mas houver um arquivo no input, tentamos liberar o botão.
                    setTimeout(() => {
                        if (fileInput.files.length > 0) setUIStatus('success');
                    }, 5000);
                }
            };

            // Função para gerenciar a UI de forma centralizada
            function setUIStatus(status) {
                ...
                if (status === 'loading') {
                    ...
                    text.innerText = "Enviando imagem ao servidor...";
                } else if (status === 'success') {
                    btn.disabled = false;
                    iconSuccess.classList.remove('hidden');
                    title.innerText = "Imagem Pronta!";
                    text.innerText = "Clique em salvar para finalizar";
                } else {
                    btn.disabled = true;
            ...
            // Monitoramento Global de Eventos de Upload (FilePond)
            // Usamos window para garantir que pegamos o evento mesmo que o DOM mude
            window.addEventListener('FilePond:addfile', () => setUIStatus('loading'));
            window.addEventListener('FilePond:processfile', (e) => {
                if (!e.detail.error) setUIStatus('success');
            });

            // ESCUTA O LIVEWIRE: Caso o Livewire receba a imagem, ele nos avisa
            @this.on('image_nutritional_updated', () => {
                 setUIStatus('success');
            });
