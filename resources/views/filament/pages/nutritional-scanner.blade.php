<x-filament-panels::page>
    ...
    <style>
        ...
        .scan-frame {
            ...
            width: min(75vw, 300px); height: 180px;
        ...
        .confirm-btn {
            background: var(--primary); color: #000; border: none; padding: 18px 32px; border-radius: 12px;
    ...
    <div class="app-container">
        {{-- IMPORTANTE: o form do Filament/Livewire deve ficar fora de wire:ignore para o upload sincronizar --}}
        ...
        <div
            ...
            x-data="{
                mode: 'confirm',
                init() {
                    @if(!empty($foundProduct?->image_nutritional))
                        this.mode = 'preview';
                    @endif

                    window.addEventListener('reset-scanner', () => {
                        this.mode = 'confirm';
                    });

                    window.addEventListener('set-preview-mode', () => {
                        this.mode = 'preview';
                    });
                }
            }"
        >
            <div class="header-info">
                <h2>{{ $foundProduct?->productname ?? 'Produto Identificado' }}</h2>
                <span>EAN: {{ $scannedCode }}</span>
            </div>

            {{-- Só a área visual fica em wire:ignore (scanner/preview). O upload não pode depender disso. --}}
            <div class="viewport-area" wire:ignore>
                ...
                <div class="preview-stage" x-show="mode === 'preview'" style="display:none">
                    ...
                    <button id="btn-retake" class="text-gray-400 text-xs mt-4 underline p-2" type="button"
                        wire:loading.attr="disabled"
                        wire:target="save,data.image_nutritional"
                    >
                        Tirar outra foto
            ...
            <div class="controls-bar">
                <button type="button" wire:click="resetScanner" class="icon-btn">
                    <x-heroicon-o-x-mark class="w-6 h-6" />
                </button>

                <div x-show="mode === 'preview'" style="display:none">
                    {{-- Botão “inteligente”: trava em upload/save e não deixa salvar sem preview --}}
                    <button
                        type="button"
                        class="icon-btn save-btn"
                        wire:click="save"
                        x-bind:disabled="mode !== 'preview'"
                        wire:loading.attr="disabled"
                        wire:target="save,data.image_nutritional"
    ...
    <script>
        document.addEventListener('livewire:initialized', () => {
            ...
            // --- Lógica de Upload (abre câmera e injeta no FileUpload) ---
            function openCamera() {
                const fileInput = document.querySelector('input[type="file"].filepond--browser');

                if (!fileInput) {
                    alert('Componente de câmera não inicializado. Recarregue a página.');
                    return;
                }
            ...
            async function startScanner() {
                // Se já existe produto encontrado, não scaneia
                if (@json((bool) $foundProduct)) return;
                ...
                try {
                    if (availableCameras.length === 0) {
                        ...
                        // Prioriza câmera traseira quando possível
