            ...
            <div class="bg-gray-50 dark:bg-gray-900 p-4 rounded-lg border border-gray-200 dark:border-gray-700 flex flex-col md:flex-row items-center justify-between gap-6">
                ...
                <div class="flex flex-col items-center">
                    <span class="text-xs font-bold text-gray-400 uppercase tracking-wider mb-2">
                        {{-- CORREÇÃO: Uso de labelLayout --}}
                        Preview: {{ $this->labelLayout === 'tabular' ? 'Tabular (Modo Duplo)' : 'Padrão (100x80mm)' }}
                    </span>
                    
                    <div class="bg-white shadow-lg border border-gray-300 relative overflow-hidden" style="width: 100mm; height: 80mm;">
                        ...
                        @else
                            <div class="grid grid-cols-2 h-full w-full">
                                <div class="border-r border-dashed border-gray-300 h-full relative flex items-center justify-center bg-gray-50">
                                    <div style="transform: rotate(90deg); width: 80mm; height: 50mm; box-shadow: 0 0 5px rgba(0,0,0,0.1);">
                                        @include('components.fda-label-tabular', ['product' => $this->product, 'settings' => $settings])
                                    </div>
                                </div>
                                <div class="h-full relative flex items-center justify-center bg-gray-50">
                                    <div style="transform: rotate(90deg); width: 80mm; height: 50mm; box-shadow: 0 0 5px rgba(0,0,0,0.1);">
                                        @include('components.fda-label-tabular', ['product' => $this->product, 'settings' => $settings])
                                    </div>
                                </div>
                ...
                <div class="flex flex-col items-end gap-2">
                    <x-filament::button
                        ...
                        x-on:click="
                            {{-- CORREÇÃO: Passamos o valor de labelLayout para o parâmetro URL 'layout' --}}
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                <div class="bg-white dark:bg-gray-800 rounded-xl shadow border border-gray-200 dark:border-gray-700 overflow-hidden flex flex-col h-full">
                    ...
                    <div class="flex-1 bg-black flex items-center justify-center p-4 relative min-h-[400px]">
                        @if($this->product->image_nutritional)
                            <img src="{{ asset('storage/' . $this->product->image_nutritional) }}" class="max-w-full max-h-[600px] object-contain rounded border border-gray-700" alt="Foto Tabela Nutricional">
                        @else
                ...
                <div class="bg-white dark:bg-gray-800 rounded-xl shadow border border-gray-200 dark:border-gray-700 overflow-hidden flex flex-col h-full">
                    ...
                    <div class="p-5 space-y-5 overflow-y-auto max-h-[600px]">
                        ...
                        {{-- Resumo Nutricional --}}
                        <div class="grid grid-cols-2 gap-4 text-sm border-t border-gray-200 pt-4">
                             <div>
                                <span class="block text-gray-500 text-xs">Serving Size</span>
                                <span class="font-bold">{{ $this->product->serving_size_quantity }}{{ $this->product->serving_size_unit }} ({{ $this->product->serving_weight }})</span>
                             </div>
                             <div>
                                <span class="block text-gray-500 text-xs">Calories</span>
                                <span class="font-bold text-xl">{{ $this->product->calories }}</span>
                             </div>
                        </div>
